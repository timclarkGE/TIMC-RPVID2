...
if platform.system() == 'Windows':
    if '64 bit' in platform.python_compiler():
        if PYINSTALLER:
            WinDLL(sys.prefix + '\gclib\dll\\x64\libcrypto-1_1-x64.dll')
            WinDLL(sys.prefix + '\gclib\dll\\x64\libssl-1_1-x64.dll')
            _gclib_path = sys.prefix + '\gclib\dll\\x64\gclib.dll'
            _gclibo_path = sys.prefix + '\gclib\dll\\x64\gclibo.dll'
        else:
